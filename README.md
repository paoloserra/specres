...
- reconstructs the spectral convolution kernel `K` from the mean autocorrelation `<A_F>` as `K = IFT{+/- sqrt[FT(<A_F>)]}`, including an algorithm for sign disambiguation and another one to minimise the impact of artefacts in the datacube;
...
```
usage: specres.py [-h] -c CUBE [-m MASK] [-nspec NR_SPEC] [-nchan NR_CHAN]
                  [-sinc SINC_KERNEL [SINC_KERNEL ...]] [-gauss GAUSS_KERNEL [GAUSS_KERNEL ...]] [-hann HANNING_KERNEL [HANNING_KERNEL ...]]
                  [-box BOXCAR_KERNEL [BOXCAR_KERNEL ...]] [-bin BINOMIAL_KERNEL [BINOMIAL_KERNEL ...]]
                  [-artlen ARTEFACTS_AUTOCORR_LENGTH] [-artord ARTEFACTS_AUTOCORR_ORDER] [-notrack]
                  ...
                  [-interp INTERP_SIGN_CHANGE INTERP_SIGN_CHANGE INTERP_SIGN_CHANGE]
                  [-floor NOISE_FLOOR]
