...
- reconstructs the spectral convolution kernel `K` from the mean autocorrelation `<A_F>` as `K = IFT{+/- sqrt[FT(<A_F>)]}`, including a sign disambiguation algorithm and one to minimise the impact of artefacts in the datacube;
