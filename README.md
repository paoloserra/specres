...
- reconstructs the spectral convolution kernel `K` from the mean autocorrelation `<A_F>` as `K = IFT{+/- sqrt[FT(<A_F>)]}`, including an algorithm for sign disambiguation and another one to minimise the impact of artefacts or real signal in the datacube (`Z` in the image below);
...
```
usage: specres.py [-h] -c CUBE [-m MASK] [-nspec NR_SPEC] [-nchan NR_CHAN] [-o OUTPUT] [-sinc SINC_KERNEL [SINC_KERNEL ...]] [-gauss GAUSS_KERNEL [GAUSS_KERNEL ...]]
